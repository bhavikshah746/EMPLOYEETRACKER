        ...
        <div class="page_header clearfix">
            ...
            <ul class="p_actions ul_reset clearfix">
                ...
                {{--<li>--}}
                    ...
                    {{--</button>--}}
                {{--</li>--}}
                @if(Sentinel::getUser()->role==1 || in_array('areas',$logged_in_user->permissions))
                    ...
                    @if((isset($admin_mode) && $admin_mode==true) || Sentinel::getUser()->role==1)
                        <li>
                            <a href="{{URL::route('admin.areas.create')}}" class="button confirm_demo">
                                Create New User Pin
                            </a>
                        </li>
                    @endif
    ...
    <script type="text/javascript">
        $('#admin_mode').change(function(e){
            if($('#admin_mode:checked').val()==1){
                window.location.href = "{!!URL::route('admin.areas.index','admin_mode=true')!!}";
            }else{
                window.location.href = "{!!URL::route('admin.areas.index')!!}";
            }
        });
        oTable = $('#users_table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{!!URL::to('admin/ajax/areas/data')!!}',
                data: {
                }
            },
            columns: [
        ...
        $(document).on('click', '.delete', function () {
            if(confirm('You Sure Want to delete This User Pin?')){
                $.ajax({
                    dataType: 'json',
                    type: 'post',
                    url: '{{URL::to('admin/ajax/areas/delete')}}',
                    data: 'area_id=' + $(this).attr('id'),
